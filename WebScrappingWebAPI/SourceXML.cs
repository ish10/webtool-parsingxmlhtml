...
namespace WebScrappingWebAPI
{
    public class SourceXML
    {
        ...
        internal static async Task toolProcessAsync()
        {
            var path = (Path.GetDirectoryName(Assembly.GetEntryAssembly().Location)).ToString().Replace(@"\WebScrappingWebAPI\bin\Debug\net5.0", @"\");
            string fileName = "";
