...
namespace MigrationToolWebAPI
{
    class HTMLUtilities
    {
        ...
        internal static void createDestHTML(string mainhtmlFile, string xmlpath, Dictionary<string, string> dr, string SourceXml, FinalPaths finalPathDest)
        {
            ...
            finalPathDest.destnationHTML = filepath;
            ...
            foreach (var child in bodyNode.ChildNodes)
            {
               if(!checkHTMLNode(child))
               {
                    ...
                    {
                        //to remove all spaces in id before using it
                        string id = destBodyNode.GetAttributeValue("id", "").ToString().Trim();
                        destBodyNode.SetAttributeValue("id", id);
                        destBodyNode.AppendChild(child);
                    }
               }
            }
